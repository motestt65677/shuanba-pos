...
<div class="ui form">
    ...
    <div class=" fields">
        <div class="eight wide field disabled">
            <label>進貨產品編號</label>
            <input id="import_no" type="text" value="">
        </div>
        <div class="eight wide field">
            <label>進貨產品名稱</label>
            <input id="import_name" type="text" value="">
        </div>
    </div>
    ...
    <div class=" fields">
        <div class="eight wide field">
            <label>說明</label>
            <input id="import_description" type="text" value="">
        </div>
        <div class="eight wide field">
            <label>價格</label>
            <input id="import_price" type="text">
        </div>
    </div>
</div>
<div class="ui segment">
    ...
    <div class="col-sm-12" >
        <table id="import_materials" style="width:100%; text-align:center;" class="ui celled table">
            <thead>
                <tr>
                    <th>序號</th>
                    <th>材料</th>
...
$(document).ready(function(){
    ...
    $("#submit").click(function(){
        if( $('.ui.form').form('is valid')) {
            let data = {
                ...
                "import_description": $("#import_description").val(),
                "import_price": $("#import_price").val(),
            };

            var table = document.getElementById("import_materials");
            var items = [];
            ...
            $.ajax({
                ...
                success: function(response) {
                    window.location.href = "/imports/index";
                },
    ...
    let row_number = 0;
    $("#add_item_btn").click(add_import_material_row);

    function add_import_material_row (data = {}){
        console.log(data);
        const table_name = "import_materials";
    ...
    function load_import(){
        return $.ajax({
            type: "POST",
            url: "/imports/queryData",
            contentType: "application/json",
            dataType: "json",
            beforeSend: showLoading,
            complete: hideLoading,
            data: JSON.stringify({"search": {"import_id": get_url_id()}}),
            success: function(response) {
                console.log(response);
                if(response["data"].length > 0){
                    const item = response["data"][0];
                    $("#import_name").val(item["import_name"]);
                    $("#import_description").val(item["import_description"]);
                    $("#import_price").val(item["import_price"]);
                }
            },
            error: function(response) {
                // console.log(response);
    ...
    function bind_import_materials(){
        return $.ajax({
            type: "POST",
            url: "/import_materials/queryData",
            contentType: "application/json",
            dataType: "json",
            beforeSend: showLoading,
            complete: hideLoading,
