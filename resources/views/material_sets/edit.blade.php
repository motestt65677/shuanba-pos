...
function bind_supplier_select(){
    return $.ajax({
        ...
        success: function(response) {
            ...
            // bind_material_select(update_form);
        },
        error: function(response) {
            // console.log(response);
        }
...
function get_material_set(){
    return $.ajax({
        ...
        beforeSend: showLoading,
        complete: hideLoading,
        data: JSON.stringify({"search": {"set_id": get_url_id()}}),
    });
}
async function init(){
    const response = await get_material_set();
    if(response["data"].length > 0){
        const set = response["data"][0];
        await bind_supplier_select();
        $("#supplier").dropdown("set selected", set["supplier_id"])
        await bind_material_select();
        $("#material").dropdown("set selected", set["material_id"]);
        $("#set_name").val(set["set_name"]);
        $("#set_unit_price").val(set["set_unit_price"]);
        $("#material_count").val(set["material_count"]);
        update_material_unit();
        update_material_unit_price();
    }    
}
